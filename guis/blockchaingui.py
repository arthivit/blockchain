...
#connect node
        ...
        self.connect_node_submit_button = tk.Button(root, text="Connect Node", command=self.submit_node, bg="pale turquoise", font="AppleMyungjo 22")
        self.connect_node_submit_button.grid_forget()
...
# button functions
    ...
    def mine_block(self):
        ...
        data = requests.get(f'http://localhost:{self.node}/mine_block')
        block = data.json()
        ...
        self.mine_label.after(3000, lambda:  self.mine_label.config(text=''))
    ...
    def get_chain(self):
        ...
        self.getchain_label.after(3000, lambda:  self.getchain_label.grid_forget())
    ...
    def connect_node(self):
        ...
        self.node_text.grid(row=5, column=1)
        self.node_text.after(20000, lambda: self.node_text.grid_forget())
        self.connect_node_submit_button.grid(row=6, column=1)
        
    def submit_node(self):
        data = {
            "nodes": self.node_text.get('1.0', 'end-1c')
        }
        self.node_text.grid_forget()
        self.connect_node_submit_button.grid_forget()
        response = requests.post(f'http://localhost:{self.node}/connect_node', json=data)

        message = response.json()['message']
        total_nodes = response.json()['total_nodes']
        self.connect_node_instruction.config(text= f"{message} {total_nodes}")
        self.connect_node_instruction.after(10000, lambda: self.connect_node_instruction.config(text=''))
